"use strict";
/*
	FOUND @: (saves me a lot of work, yeahhh!)
...
{
    assert(arrayBuffer instanceof ArrayBuffer, "ArrayBuffer is null or incompatible!");
    this.pos = 0;
...
BinaryReader.prototype = {
    seek: function(pos) {
        assert(pos >=0 && pos <= this.data.length ,'Reader seek, failed!');
        var oldPos = this.pos;
        this.pos = pos;
        return oldPos;
    },
    ...
    getUint8: function() {
        assert(this.pos < this.data.length,'getUint8, failed!');
...
TrueTypeFont.prototype = {
        ...
        for( var i = 0 ; i < numTables; i++ ) {
            ...
            if (tag !== 'head') {
                assert(this.calculateTableChecksum(file, tables[tag].offset,
                            tables[tag].length) === tables[tag].checksum, 'TTF, no head found!');
            }
        }
    ...
    readHeadTable: function(file) {
        ...
        assert(this.magicNumber === 0x5f0f3cf5,'magic number error!');
    ...
    glyphCount: function() {
        assert("maxp" in this.tables, 'glyphCount error!');
    ...
    getGlyphOffset: function(index) {
        assert("loca" in this.tables, 'glyph offset error!');
